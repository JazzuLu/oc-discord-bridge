...
- `DISCORD_CWD_ALLOW_ROOTS` (optional, comma-separated absolute roots allowed for channel `CWD=`; when unset/empty, any absolute path is allowed)
- `DISCORD_CWD_REQUIRE_EXISTS` (default: `true`; when `true`, CWD must exist and be a directory)
- `DISCORD_ALLOWED_CWD_PREFIXES` (deprecated alias for `DISCORD_CWD_ALLOW_ROOTS`)
